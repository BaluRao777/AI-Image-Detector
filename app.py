...
def predict_multiple(files):
    ...
    # 🔥 Handle folder vs files
    for f in files:
        if os.path.isdir(f.name):
            # If folder → collect all images inside
            for root, _, filenames in os.walk(f.name):
                for file in filenames:
                    if file.lower().endswith((".jpg", ".jpeg", ".png")):
                        expanded_files.append(
                            type("obj", (), {"name": os.path.join(root, file)})
                        )
        else:
    ...
    for f in expanded_files:
        try:
            ...
            if img is None:
                results.append(f"{f.name} → Invalid Image")
                continue

            result = safe_predict(img)

            results.append(
                f"{os.path.basename(f.name)} → {result['label']} ({result['confidence']*100:.1f}%)"
            )

        except Exception as e:
            print("BATCH ERROR:", e)
            results.append(f"{f.name} → Error: {str(e)}")
